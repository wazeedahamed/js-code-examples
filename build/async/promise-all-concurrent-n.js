...
var promiseAllConcurrent = (function () {
    ...
    function concurrentProcess(limit, list) {
        ...
        return limit >= total ?
            Promise.all(list.map(function (promise) { return promise(); })) :
            new Promise(function (resolve) {
                next({ concurrent: 0, queued: 0, queue: [] });
                function next(meta, previous) {
                    // previous parameter is used only for logging
                    var queue = meta.queue, queued = meta.queued, concurrent = meta.concurrent;
                    if (queued == total)
                        return concurrent == 0 ? complete(queue) : null;
                ...
                function concurrency(flag, meta) {
    ...
    return function (limit) { return function (list) { return concurrentProcess(limit, list); }; };
}());
/** Testing code **/
